...
void Utilities::set(SymbolTable &newSymbols) {
    symbolsTable = newSymbols;
}

string Utilities::shuntingYard(vector<string> instream, SymbolTable & symbols) {
    stack<string> operationStack;
    queue<string> outPutQueue;
    cout << "Welcome to Shunting_Yard :: with symbolTable size " <<  symbols.size() << "Start read instream\n";
    for (std::vector<string>::iterator it = instream.begin(); it != instream.end(); ++it) {
        ...
        if (operators[*it] != nullptr) {
            ...
            while (!operationStack.empty() && operators[operationStack.top()] != nullptr && !precedence(*it)) {
                cout << "1. So operator stack is not empty and " << *it
                     << " is less or equal to whatever is in stack\n";
            ...
            if (symbols.isInMap(*it)){
                cout << "added " << symbols.get(*it) << "\n";
